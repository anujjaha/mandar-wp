...
class pt_search_widget extends WP_Widget {
	...
	?>
		...
		<p>
		    <input type="checkbox" name="<?php echo $this->get_field_name('search-product'); ?>" <?php if (esc_attr( $instance['search-product'] )) {
		                    echo 'checked="checked"';
		                } ?> class=""  size="4"  id="<?php echo $this->get_field_id('search-product'); ?>" />
		    <label for ="<?php echo $this->get_field_id('search-product'); ?>"><?php esc_html_e('Product search only','storex'); ?></label>
		</p>
	<?php
	}
